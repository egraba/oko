...
pub fn collect_machine_info(device: &mut Device) {
    ...
    device.hw_serial_number = NOT_IMPLEMENTED.to_string();
    device.hw_type = NOT_IMPLEMENTED.to_string();
    device.hw_model = NOT_IMPLEMENTED.to_string();

    device.net_host_name = sys.host_name().expect(NOT_RETRIEVED);
    device.net_ipv4 = NOT_IMPLEMENTED.to_string();
    device.net_mac = NOT_IMPLEMENTED.to_string();

    device.cpu_arch = NOT_IMPLEMENTED.to_string();
